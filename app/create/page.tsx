...
export default function Create(){
  ...
  async function authSession(){const {data:{session}}=await supabase.auth.getSession();if(!session){window.location.href='/login';throw new Error('Session expirée')}return session}
  ...
  async function start(e:React.FormEvent){
    e.preventDefault();if(!hasAccess){setGate(true);return}if(!businessName.trim()){setMsg('Ajoutez le nom de votre entreprise.');return}if(!whatsappNumber){setMsg('Ajoutez votre numéro WhatsApp.');return}
    setLoading(true);setMsg('');
    try{
      const session=await authSession();const user=session.user;let catalogPayload:any;
      const businessContext={name:businessName.trim(),business_type:selectedPreset?.business_type||'other',currency_code:'XOF',country_code:'CI',language:'fr',phone_whatsapp:whatsappNumber};
      if(mode==='blank'){
        ...
        catalogPayload=ocrData.catalog;catalogPayload.business={...(catalogPayload.business||{}),...businessContext};catalogPayload.catalog={...(catalogPayload.catalog||{}),title:title.trim()||businessName.trim(),type:selectedPreset?.catalog_type||catalogPayload.catalog?.type||'catalog'};
      }
      ...
      if(trialActive)localStorage.setItem('qatalink_trial_reminder_on_arrival','1');window.location.href=`/dashboard?tab=items&catalog=${encodeURIComponent(saved.catalog_id)}&created=1`;
  ...
  return <div className="auth-wrap create-wrap"><div className="auth-card create-card">
    <Link href="/dashboard" className="brand"><Image src="/qatalink-logo.png" width={34} height={34} alt="Qatalink"/>qatalink</Link><h1>Créez votre catalogue</h1><p style={{color:'var(--muted)'}}>{trialActive?'Votre essai 24 h vous donne accès à toute la création.':'Importez une carte existante ou partez de zéro.'}</p>
    ...
    <div className="field"><label>WhatsApp de l’entreprise</label><div style={{display:'grid',gridTemplateColumns:'minmax(138px,.45fr) minmax(0,1fr)',gap:8}}><select className="input" value={dialCode} onChange={e=>setDialCode(e.target.value)}>{countryCodes.map(c=><option key={c[0]} value={c[2]}>{c[1]} {c[2]}</option>)}</select><input className="input" inputMode="tel" value={whatsappLocal} onChange={e=>setWhatsappLocal(digitsOnly(e.target.value))} placeholder="0700000000"/></div><small className="field-help">Le numéro est enregistré sans espaces au format {whatsappNumber||`${dialCode}…`}.</small></div>
    ...
    <form className="form" onSubmit={start}><div className="field"><label>Nom du catalogue</label><input className="input" value={title} onChange={e=>{setTitleTouched(true);setTitle(e.target.value)}} placeholder={businessName||selectedPreset?.default_catalog_title||'Catalogue'}/><small className="field-help">Par défaut, Qatalink reprend le nom de votre entreprise. Vous pouvez le modifier.</small></div>
      ...
      {msg&&<div className={loading?'progress-note':'error'}>{msg}</div>}<button className="btn btn-primary" disabled={loading}>{loading?'Traitement en cours…':mode==='blank'?'Créer mon catalogue':'Générer mon Qatalink'}</button>
    </form>
