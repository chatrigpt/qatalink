...
export async function POST(req:NextRequest){
  try{
    ...
    for(const itemId of ids){
      ...
      if(!provider.ok){jobs.push({item_id:itemId,error:providerData?.error?.message||providerData?.error||'PoYo submit failed'});continue;}
      const taskId=providerData?.data?.task_id;
      if(!taskId){jobs.push({item_id:itemId,error:'PoYo did not return task_id'});continue;}

      const {data:job,error:jobError}=await supabase.from('item_image_generation_jobs').insert({business_id:catalog.business_id,item_id:item.id,prompt,status:'processing',provider:'poyo:gpt-image-2',provider_task_id:taskId,provider_payload:providerData}).select('id').single();
      if(jobError||!job){jobs.push({item_id:itemId,error:jobError?.message||'Could not save generation job'});continue;}
      jobs.push({item_id:itemId,job_id:job.id,task_id:taskId,status:'processing'});
    }
