...
const GENERATED_IMAGE_MAX_EDGE=1400;
const GENERATED_IMAGE_WEBP_QUALITY=78;
...
export async function POST(req:NextRequest){
  try{
    ...
    const body=await req.json();const jobIds=[...new Set((Array.isArray(body?.job_ids)?body.job_ids:[body?.job_id]).filter(Boolean))];if(!jobIds.length)return NextResponse.json({success:false,error:'NO_JOBS'},{status:400});
    const results:any[]=[];
    for(const jobId of jobIds){
      ...
      let bytes:Buffer;
      try{bytes=await compressGeneratedImage(sourceBytes)}catch(error){console.error('[Qatalink:ImageCompression]',{job_id:job.id,error});results.push({job_id:job.id,item_id:job.item_id,status:'processing',progress:100});continue}
      const storagePath=`${user.id}/${job.item_id}/${Date.now()}-${job.id}.webp`;
