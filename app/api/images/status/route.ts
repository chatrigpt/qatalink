...
export async function POST(req:NextRequest){
  try{
    ...
    for(const jobId of jobIds){
      ...
      if(job.status==='failed'){results.push({job_id:job.id,item_id:job.item_id,status:'failed',error:job.error_message});continue;}
      if(!job.provider_task_id){results.push({job_id:job.id,item_id:job.item_id,status:'failed',error:'Missing provider task id'});continue;}
      ...
      if(task.status==='failed'){
        await supabase.from('item_image_generation_jobs').update({status:'failed',error_message:task.error_message||'Generation failed',provider_payload:providerData,completed_at:new Date().toISOString()}).eq('id',job.id);
        results.push({job_id:job.id,item_id:job.item_id,status:'failed',error:task.error_message||'Generation failed'});
        continue;
      }
      ...
      results.push({job_id:job.id,item_id:job.item_id,status:'completed',image_url:imageUrl});
    }
