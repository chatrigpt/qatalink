...
export async function POST(req:NextRequest){
  try{
    ...
    const lat=Number(body?.lat),lng=Number(body?.lng);if(!Number.isFinite(lat)||!Number.isFinite(lng)||lat<-90||lat>90||lng<-180||lng>180||(Math.abs(lat)<1e-8&&Math.abs(lng)<1e-8))return NextResponse.json({success:false,error:'INVALID_COORDINATES'},{status:400});
