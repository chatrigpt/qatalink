...
export async function POST(req:NextRequest){
  try{
    ...
    const {data:catalog,error:catalogError}=await supabase.from('catalogs').select('id,business_id,title,public_slug,hub_public_slug,description,display_name,logo_url').eq('id',catalogId).maybeSingle();if(catalogError||!catalog)return NextResponse.json({success:false,error:'CATALOG_UNAVAILABLE'},{status:404});
    ...
    const exactDestinations=qrMode==='catalog'?`The real QR will encode exactly this active catalogue URL: ${catalogUrl}`:qrMode==='hub'?`The real QR will encode exactly this active central-page URL: ${hubUrl}`:`Two real QR codes will be overlaid: catalogue URL ${catalogUrl} and central-page URL ${hubUrl}.`;
