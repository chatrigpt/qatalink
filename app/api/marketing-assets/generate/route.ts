...
function safeColor(v:unknown,fallback:string){const s=clean(v,20);return /^#[0-9a-f]{6}$/i.test(s)?s:fallback}
...
export async function POST(req:NextRequest){
  try{
    ...
    const body=await req.json();const catalogId=clean(body?.catalog_id,80);const rawAsset=clean(body?.asset_type,30);const assetType=rawAsset==='tent_card'?'tent_card':rawAsset==='simple_card'?'simple_card':'flyer';const generationMode=body?.generation_mode==='custom'?'custom':'auto';const qrMode=['hub','catalog','both'].includes(body?.qr_mode)?body.qr_mode:'catalog';const customPrompt=generationMode==='custom'?clean(body?.custom_prompt,1800):'';const selectedIds=[...new Set((Array.isArray(body?.reference_item_ids)?body.reference_item_ids:[]).map((x:any)=>clean(x,80)).filter(Boolean))].slice(0,6);
    ...
    const [{data:business},{data:wallet},{data:sub},{data:categories},{data:itemRows},{data:theme},{data:override}]=await Promise.all([
      ...
      supabase.from('catalog_theme_settings').select('primary_color,secondary_color,background_color,text_color,background_mode,background_gradient').eq('catalog_id',catalogId).maybeSingle(),
      supabase.from('account_access_overrides').select('unlimited_credits,all_features').eq('user_id',user.id).maybeSingle()
    ]);
    if(!business)return NextResponse.json({success:false,error:'BUSINESS_UNAVAILABLE'},{status:404});
    const validSub=!!sub&&['active','trialing'].includes(String(sub.status))&&(!sub.current_period_end||new Date(sub.current_period_end).getTime()>Date.now());if(!validSub&&!override?.all_features)return NextResponse.json({success:false,error:'SUBSCRIPTION_REQUIRED'},{status:402});
    const cost=generationMode==='custom'?CUSTOM_COST:AUTO_COST;const balance=Number(wallet?.balance||0);if(!override?.unlimited_credits&&balance<cost)return NextResponse.json({success:false,error:'INSUFFICIENT_CREDITS',balance,required:cost,message:`Il faut ${cost} crédits pour générer ce support.`},{status:402});
    ...
    const primary=safeColor(theme?.primary_color,'#B5122B'),secondary=safeColor(theme?.secondary_color,'#F7E8EA'),background=safeColor(theme?.background_color,'#FFFFFF'),text=safeColor(theme?.text_color,'#171719');const palette=`PRIMARY ${primary}; SECONDARY ${secondary}; BACKGROUND ${background}; TEXT/INK ${text}`;
    ...
    const automaticDirection=`Automatic campaign direction: create a visually spectacular premium advertising hero, with cinematic depth, strong editorial composition, sophisticated lighting, dynamic layering and a clear hero subject. It should feel like a high-end commercial campaign rather than a basic product collage. Use supplied logo/product photographs faithfully and highlight a varied selection across categories without overcrowding. GRAPHIC BACKGROUNDS, LIGHTING TINTS, GRADIENTS, FRAMES AND ACCENTS MUST USE ONLY THE CATALOGUE PALETTE LISTED BELOW. Natural colors of real food, skin, clothing, products and packaging may remain faithful to the references, but do not introduce unrelated decorative colors.`;
    const prompt=`Create a premium LANDSCAPE commercial hero image for the upper visual area of a printable scan card for a real business in Côte d’Ivoire. The generated artwork itself is NOT the final flyer: the application will add typography, CTA and real QR code(s) separately.\n\nBusiness name: ${businessName}\nSector: ${business.business_type||'commerce'}\nCatalogue: ${clean(catalog.title,120)}\nBusiness positioning: ${bio}\nCategories: ${categoryNames.join(', ')||'Not provided'}\nFeatured offers: ${featured.join(', ')||'Not provided'}\nCATALOGUE COLOR PALETTE: ${palette}\n${exactDestinations}\n${fallbackText}\n${generationMode==='custom'?`User visual direction: ${customPrompt}. Respect the catalogue palette for all graphic styling and ambience.`:automaticDirection}\n\nMANDATORY VISUAL RULES:\n- LANDSCAPE advertising composition optimized for 16:9; 4:3 may be used only as provider fallback.\n- Keep all essential products, dishes, faces and distinctive details inside the CENTRAL 80% safe zone.\n- The final printable layout uses the selected catalogue palette. Do not invent a separate brand palette.\n- Use supplied reference images faithfully. Never invent a different logo, packaging or product identity.\n- Strong visual hierarchy: one main hero subject with supporting items, premium editorial/commercial photography, realistic light and materials.\n- Do NOT reserve a fake white footer and do NOT attempt to design the final flyer yourself.\n- The application generates the real scannable QR code(s) programmatically. Do NOT draw, fake, imitate or include any QR code yourself.\n- Do NOT render any barcode, URL, phone number, price, headline, logo text, CTA text, menu text or other readable typography in the generated image.\n- No watermark, no fake UI, no poster frame, no AI artifacts.\n- Visual tone should fit the business sector and feel locally credible for Abidjan / Côte d’Ivoire.`;
    ...
    await supabase.from('marketing_asset_generation_jobs').update({status:'processing',provider:`poyo:${submitted.model}`,provider_task_id:submitted.taskId,provider_payload:{...(submitted.data||{}),requested_size:submitted.size,catalog_url:catalogUrl,hub_url:hubUrl||null,palette:{primary,secondary,background,text}}}).eq('id',job.id);
    return NextResponse.json({success:true,job_id:job.id,status:'processing',credit_cost:cost,balance:Number(newBalance),catalog_url:catalogUrl,hub_url:hubUrl||null,business_name:businessName,bio,cta:ctaFor(String(business.business_type||'')),reference_count:referenceUrls.length,generated_ratio:submitted.size,palette:{primary,secondary,background,text}});
