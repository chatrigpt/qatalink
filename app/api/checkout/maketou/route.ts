...
export async function POST(req:NextRequest){
  ...
  try{
    ...
    const {plan,firstName,lastName}=await req.json();
    ...
    const payload={
      ...
      customerPrice:prices[p],
      redirectURL:`${appUrl}/dashboard?payment=pending`,
      meta:{user_id:user.id,business_id:businessId,plan:p,plan_code:planCodes[p],billing_period:'monthly'}
    };
    ...
    if(cartId){
      const {error:paymentError}=await supabase.from('payments').insert({business_id:businessId,plan_code:planCodes[p],provider:'maketou',provider_cart_id:cartId,amount_minor:prices[p],currency_code:'XOF',status:'pending',customer_email:user.email||null,raw_provider_response:data});
