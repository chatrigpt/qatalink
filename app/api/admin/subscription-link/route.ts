...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  const product=products[plan][billingPeriod];const origin=(req.headers.get('origin')||process.env.NEXT_PUBLIC_APP_URL||new URL(req.url).origin).replace(/\/$/,'');const returnToken=crypto.randomUUID();const local=String(target.email||email).split('@')[0].replace(/[._-]+/g,' ').trim();const names=local.split(/\s+/).filter(Boolean);const payload={productDocumentId:product.id,email:target.email||email,firstName:names[0]||'Client',lastName:names.slice(1).join(' ')||'Qatalink',redirectURL:`${origin}/payment/return?kind=subscription&direct=${encodeURIComponent(returnToken)}`,meta:{user_id:target.user_id,business_id:target.business_id,plan,plan_code:planCodes[plan],billing_period:billingPeriod,period_months:product.months,created_by_admin:user.id,public_return_token:returnToken}};
  ...
  if(cartId){const {error}=await supabase.from('payments').insert({business_id:target.business_id,plan_code:planCodes[plan],provider:'maketou',provider_cart_id:cartId,amount_minor:product.amount,currency_code:'XOF',status:'pending',customer_email:target.email||email,billing_period:billingPeriod,period_months:product.months,public_return_token:returnToken,raw_provider_response:data});if(error&&error.code!=='23505')return NextResponse.json({error:'PAYMENT_TRACKING_FAILED'},{status:500})}
  if(!redirectUrl)return NextResponse.json({error:'PAYMENT_LINK_UNAVAILABLE',cart_id:cartId},{status:502});return NextResponse.json({success:true,url:redirectUrl,cart_id:cartId,email:target.email,business_id:target.business_id,plan,billing_period:billingPeriod,amount:product.amount,return_token:returnToken});
