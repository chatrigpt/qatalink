...
export function OpsDeliveryTracking(){
    ...
    async function refresh(){
      ...
      try{const r=await fetch('/api/ops/orders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({access_key:accessKey,pin,action:'list',limit:150}),cache:'no-store'});const data=await r.json();if(!r.ok||stopped)return;const byNumber=new Map<string,any>((data.orders||[]).map((o:any)=>[String(o.order_number),o]));
        document.querySelectorAll<HTMLElement>('.ops-order').forEach(card=>{
          ...
          if(order.flow_mode!=='delivery')return;const footer=card.querySelector<HTMLElement>('footer');if(!footer||footer.querySelector('[data-delivery-start]'))return;const btn=document.createElement('button');btn.type='button';btn.className='ops-delivery-button';btn.setAttribute('data-delivery-start','1');btn.innerHTML='🚚 <span>Démarrer livraison</span>';btn.onclick=async()=>{const label=window.prompt('Nom ou repère du livreur (facultatif) :','')??'';try{btn.setAttribute('disabled','true');btn.innerHTML='🚚 <span>Préparation…</span>';const res=await fetch('/api/ops/delivery',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({access_key:accessKey,pin,action:'start',order_id:order.id,driver_label:label}),cache:'no-store'});const j=await res.json();if(!res.ok)throw new Error(j?.error||'DELIVERY_FAILED');setModal({order:number,driver:j.driver_url,tracking:j.tracking_url});btn.innerHTML='🚚 <span>Liens de suivi</span>';btn.removeAttribute('disabled')}catch(e:any){btn.innerHTML='🚚 <span>Démarrer livraison</span>';btn.removeAttribute('disabled');alert(String(e?.message||'Impossible de démarrer le suivi.').includes('BUSINESS_PLAN_REQUIRED')?'Le suivi GPS des livraisons est réservé à Business.':e?.message||'Impossible de démarrer le suivi.')}};footer.appendChild(btn);
        });
      }catch{}
    }
  ...
  return <div className="ops-delivery-modal-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)setModal(null)}}><section className="ops-delivery-modal"><button className="ops-delivery-close" onClick={()=>setModal(null)}>×</button><span className="ops-delivery-kicker">BUSINESS · LIVRAISON</span><h2>{modal.order}</h2><p>Envoyez le premier lien au livreur. Le second est celui que le client utilise pour suivre sa commande et voir le GPS lorsqu’elle est en livraison.</p><div className="ops-delivery-link"><b>Lien livreur</b><code>{modal.driver}</code><div><button onClick={()=>copy(modal.driver)}>Copier</button><a href={modal.driver} target="_blank" rel="noreferrer">Ouvrir</a></div></div><div className="ops-delivery-link customer"><b>Lien client</b><code>{modal.tracking}</code><div><button onClick={()=>copy(modal.tracking)}>Copier</button><a href={modal.tracking} target="_blank" rel="noreferrer">Ouvrir</a></div></div><small>Le livreur doit autoriser la géolocalisation précise sur son téléphone. Le client ne voit sa position que pendant l’étape « En cours de livraison ».</small></section></div>;
}
