...
import {AlertTriangle,Boxes,FileText,History,ImageUp,Link2,PackagePlus,RefreshCw,Trash2,UploadCloud,X} from 'lucide-react';
...
type ImportRow={name:string;unit:string;quantity:number;low_stock_threshold:number;note?:string};
...
export function StockManagementCenter(){
  ...
  async function saveThreshold(stock:Stock,value:number){const {error}=await supabase.from('inventory_stock_items').update({low_stock_threshold:Math.max(0,value),updated_at:new Date().toISOString()}).eq('id',stock.id);if(error)alert(error.message);else setStocks(v=>v.map(s=>s.id===stock.id?{...s,low_stock_threshold:Math.max(0,value)}:s))}
  async function deleteStock(stock:Stock){if(!confirm(`Supprimer le stock « ${stock.name} » et ses liaisons ?`))return;const {error}=await supabase.from('inventory_stock_items').delete().eq('id',stock.id);if(error)alert(error.message);else await loadAll()}
  async function addRecipe(){if(!businessEnabled||!businessId||!recipe.item_id||!recipe.stock_item_id||Number(recipe.quantity)<=0)return;setBusy('recipe');const {error}=await supabase.from('inventory_recipe_components').upsert({business_id:businessId,item_id:recipe.item_id,stock_item_id:recipe.stock_item_id,quantity_per_item:Number(recipe.quantity),updated_at:new Date().toISOString()},{onConflict:'item_id,stock_item_id'});setBusy('');if(error)alert(error.message);else await loadAll()}
