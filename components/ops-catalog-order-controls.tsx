...
export function OpsCatalogOrderControls({accessKey}:{accessKey:string}){
  ...
  async function orderedGroup(categoryId:string|null){return [...(payload?.editor?.items||[])].filter(x=>x.category_id===categoryId)}
  async function moveItem(categoryId:string|null,index:number,delta:number){const group=await orderedGroup(categoryId);const target=index+delta;if(target<0||target>=group.length)return;[group[index],group[target]]=[group[target],group[index]];await reorder('item_reorder',group.map(x=>x.id))}
  async function setItemPosition(categoryId:string|null,itemId:string,position:number){const group=await orderedGroup(categoryId);const current=group.findIndex(x=>x.id===itemId);if(current<0||busy)return;const target=Math.max(0,Math.min(group.length-1,Math.round(position)-1));if(target===current)return;const [moved]=group.splice(current,1);group.splice(target,0,moved);await reorder('item_reorder',group.map(x=>x.id))}
