...
export function LandingAndroidBetaDownload(){
  ...
  useEffect(()=>{setShow(location.pathname==='/')},[]);
  if(!show)return null;
  return <section className="android-beta-download" aria-label="Application Android Qatalink bêta">
    <div className="android-beta-inner">
      ...
      <div className="android-beta-copy"><span>APPLICATION ANDROID · BÊTA</span><h2>Emportez votre caisse et vos commandes avec vous.</h2><p>Téléchargez l’APK bêta officiel Qatalink pour Android. Il est compilé automatiquement depuis le dépôt officiel Qatalink, sans installateur tiers. L’application utilise les mêmes comptes et espaces que le site.</p><div className="android-beta-points"><b><ShieldCheck/>APK officiel Qatalink</b><b><Smartphone/>Pensé pour Android</b><b><ShieldCheck/>Connexion sécurisée à qatalink.com</b></div><small>Android peut afficher un avertissement lors de l’installation parce que cette version bêta est distribuée directement, en dehors du Play Store. Vérifiez que le fichier téléchargé s’appelle bien « qatalink-android-beta.apk ».</small></div>
      <a className="android-beta-button" href={APK_URL}><Download/>Télécharger l’APK bêta</a>
    </div>
    <style jsx>{`
      ...
      .android-beta-button{display:inline-flex;align-items:center;justify-content:center;gap:8px;text-decoration:none;background:var(--red);color:#fff;font-weight:900;border-radius:16px;padding:15px 18px;white-space:nowrap}.android-beta-button svg{width:18px}
