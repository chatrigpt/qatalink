  ...
  useEffect(()=>{
    ...
    function markRequired(label:HTMLLabelElement|null){
      ...
      if(!label.querySelector('.q-delivery-required')){
        ...
        star.className='q-delivery-required';
        star.textContent=' *';
    ...
    function ensureGpsUi(){
      ...
      if(!fields)return;
      const phoneLabel=findFieldLabel('phone');
      const addressLabel=findFieldLabel('address');
      markRequired(phoneLabel);markRequired(addressLabel);
      if(fields.querySelector('[data-q-delivery-gps]')){updateSubmit();return}

      const box=document.createElement('section');
      box.className='q-delivery-gps';
      box.setAttribute('data-q-delivery-gps','1');
      ...
      const button=box.querySelector<HTMLButtonElement>('.q-delivery-gps-button')!;
      const status=box.querySelector<HTMLElement>('.q-delivery-gps-status')!;
      button.onclick=()=>{
        ...
        navigator.geolocation.getCurrentPosition(pos=>{
          ...
          status.textContent=`Position enregistrée · précision ≈ ${Math.round(pos.coords.accuracy)} m`;
          status.classList.add('success');
          button.disabled=false;button.textContent='Actualiser ma position';
          updateSubmit();
        },err=>{
          status.textContent=err.code===1?'Autorisez la localisation précise pour continuer.':'Impossible de récupérer votre position. Activez le GPS et réessayez.';
          status.classList.add('error');button.disabled=false;button.textContent='Réessayer';updateSubmit();
      ...
      fields.querySelectorAll('input,textarea').forEach(el=>el.addEventListener('input',updateSubmit));
      updateSubmit();
    }

    function updateSubmit(){
      const flow=document.querySelector<HTMLElement>('.public-v2-flow');
      if(!flow)return;
      const submit=flow.querySelector<HTMLAnchorElement>(':scope > a');
      if(!submit)return;
      if(!deliverySelected())return;
      const phone=findFieldLabel('phone')?.querySelector<HTMLInputElement>('input')?.value.trim()||'';
      const address=findFieldLabel('address')?.querySelector<HTMLInputElement>('input,textarea')?.value.trim()||'';
      const ok=!!phone&&!!address&&!!point;
      submit.classList.toggle('q-delivery-incomplete',!ok);
      submit.setAttribute('aria-disabled',ok?'false':'true');
      submit.title=ok?'':'Renseignez le téléphone, l’adresse et votre position GPS.';
    }
      ...
      if(url.includes('/api/orders/public')&&init?.method?.toUpperCase()==='POST'&&typeof init.body==='string'){
        try{
          ...
          if(String(body?.flow_mode||'')==='delivery'){
            ...
            if(point){body.flow_fields.gps_lat=String(point.lat);body.flow_fields.gps_lng=String(point.lng);body.flow_fields.gps_accuracy=String(point.accuracy);body.flow_fields.gps_captured_at=point.capturedAt}
    ...
    const refresh=()=>{if(deliverySelected())ensureGpsUi()};
    observer=new MutationObserver(refresh);observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});
    document.addEventListener('click',clickGuard,true);document.addEventListener('input',updateSubmit,true);refresh();
