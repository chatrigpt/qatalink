...
export function PublicDeliveryLocationCapture(){
  ...
  useEffect(()=>{
    if(!location.pathname.startsWith('/c/'))return;
    const clickGuard=(event:MouseEvent)=>{if(!deliverySelected())return;const target=event.target;if(!(target instanceof Element))return;const submit=target.closest('.public-v2-confirm-order');if(!submit)return;const phoneInput=findFieldInput('phone');const addressInput=findFieldInput('address');const missingPhone=!!phoneInput?.required&&!phoneInput.value.trim();const missingAddress=!!addressInput?.required&&!addressInput.value.trim();if(!missingPhone&&!missingAddress)return;event.preventDefault();event.stopImmediatePropagation();if(missingPhone&&missingAddress)alert('Renseignez les champs obligatoires : téléphone et adresse de livraison.');else if(missingPhone)alert('Le numéro de téléphone est obligatoire pour cette livraison.');else alert('L’adresse de livraison est obligatoire. Vous pouvez aussi utiliser votre position GPS exacte.')};
