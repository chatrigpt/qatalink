...
const PERMISSIONS:[string,string][]=[
  ...
  ['can_manage_hub','Gérer la page centrale'],
];
...
export function DashboardPosCenter(){
  ...
  const [newAccess,setNewAccess]=useState<any>({label:'Équipe caisse',pin:'',can_view_revenue:true,can_view_catalog_stats:false,can_update_status:true,can_cancel_orders:false,can_print:true,can_merge_orders:true,can_edit_categories:false,can_edit_items:false,can_edit_photos:false,can_edit_prices:false,can_generate_images:false,can_use_whatsapp:true,can_manage_hub:false});
  ...
  return <div className="qpos-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><section className="qpos-modal">
    ...
    {busy==='load'||busy==='workspace'?<div className="qpos-loading">Chargement…</div>:locked?<div className="qpos-lock"><ShoppingCart/><h3>Point de vente disponible avec Pro et Business</h3><p>La configuration reste visible, mais la création des accès équipe et l’exploitation des commandes nécessitent une formule compatible.</p><a href="/#pricing">Voir les formules</a></div>:c&&<div className="qpos-body">
      ...
      <section className="qpos-card"><div className="qpos-card-title"><Users/><div><h3>Accès équipe</h3><p>Créez un lien et un code PIN par poste, serveur, caisse ou responsable. « Gérer la page centrale » est désactivé par défaut et peut être accordé indépendamment des autres droits.</p></div></div><div className="qpos-grid"><label>Nom de l’accès<input value={newAccess.label} onChange={e=>setNewAccess({...newAccess,label:e.target.value})}/></label><label>Code PIN<input type="password" minLength={4} value={newAccess.pin} onChange={e=>setNewAccess({...newAccess,pin:e.target.value})} placeholder="4 caractères minimum"/></label></div><div className="qpos-permissions">{PERMISSIONS.map(([key,label])=><label key={key}><input type="checkbox" checked={!!newAccess[key]} onChange={e=>setNewAccess({...newAccess,[key]:e.target.checked})}/>{label}</label>)}</div><button className="qpos-primary" onClick={createAccess} disabled={busy==='access'||newAccess.pin.length<4}><Plus/>Créer l’accès</button>
