...
import {CashRegister,Copy,ExternalLink,Plus,Settings2,Trash2,Users,X} from 'lucide-react';
...
export function DashboardPosCenter(){
  ...
  const [newAccess,setNewAccess]=useState<any>({label:'Équipe caisse',pin:'',can_view_revenue:true,can_update_status:true,can_cancel_orders:false,can_print:true,can_merge_orders:true,can_edit_categories:false,can_edit_items:false,can_edit_photos:false,can_edit_prices:false,can_generate_images:false,can_use_whatsapp:true});
  ...
  async function toggleAccess(row:any){setBusy(`access-${row.id}`);const {error}=await supabase.rpc('qatalink_update_team_access',{p_access_id:row.id,p_payload:{enabled:!row.enabled}});setBusy('');if(error)setNotice(error.message);else await loadWorkspace(catalogId)}
  ...
  return <div className="qpos-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><section className="qpos-modal">
    ...
    {busy==='load'||busy==='workspace'?<div className="qpos-loading">Chargement…</div>:locked?<div className="qpos-lock"><CashRegister/><h3>Point de vente disponible avec Pro et Business</h3><p>La configuration reste visible, mais la création des accès équipe et l’exploitation des commandes nécessitent une formule compatible.</p><a href="/#pricing">Voir les formules</a></div>:c&&<div className="qpos-body">
