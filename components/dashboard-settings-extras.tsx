...
import {BarChart3,ImageOff,Lock,PackageCheck,Save,ShieldCheck,Store,TrendingUp} from 'lucide-react';
import {createSupabaseBrowserClient} from '@/lib/supabase';

const BUSINESS_TYPES=[
  ['retail','Boutique / Shopping'],
  ['restaurant','Restaurant / Alimentation'],
  ['spa_beauty','Beauté & bien-être'],
...
export function DashboardSettingsExtras(){
  ...
  const [businessId,setBusinessId]=useState('');const [businessType,setBusinessType]=useState('other');const [businessTypeBusy,setBusinessTypeBusy]=useState(false);const [businessTypeNotice,setBusinessTypeNotice]=useState('');
  ...
  useEffect(()=>{if(!settingsHost)return;let alive=true;const supabase=createSupabaseBrowserClient();void (async()=>{const {data:{session}}=await supabase.auth.getSession();if(!session||!alive)return;const {data}=await supabase.from('businesses').select('id,business_type').eq('owner_user_id',session.user.id).order('created_at',{ascending:true}).limit(1).maybeSingle();if(data&&alive){setBusinessId(String(data.id||''));setBusinessType(String(data.business_type||'other'))}})();return()=>{alive=false}},[settingsHost]);
