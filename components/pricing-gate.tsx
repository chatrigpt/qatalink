...
import { Check, Clock3, Smartphone, X } from 'lucide-react';
...
export function PricingGate({open,onClose,title='Choisissez votre formule pour continuer',trialExpiresAt=null,trialActive=false}:Props){
  ...
  useEffect(()=>{
    const mark=(event:MouseEvent)=>{
      ...
      if(/s[’']?abonner|voir les formules|payer mon abonnement|abonnement supérieur|garder|conserver mon qatalink|choisir basic|choisir interactif|choisir vitrine/.test(text))setExplicitIntentAt(Date.now());
    };
  ...
  async function checkout(plan:'basic'|'interactive'|'vitrine'){
    ...
    try{
      ...
      const r=await fetch('/api/checkout/maketou',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({plan,firstName,lastName,billingPeriod})});const data=await r.json();if(!r.ok)throw new Error('Impossible de lancer le paiement Mobile Money. Réessayez.');const redirectUrl=data.redirectUrl||data.redirectURL;const cartId=data.cart?.id||data.id;if(cartId)localStorage.setItem('qatalink_maketou_cart_id',cartId);if(!redirectUrl)throw new Error('Impossible d’ouvrir le paiement Mobile Money. Réessayez.');window.location.href=redirectUrl;
    }catch{setError('Le paiement Mobile Money n’a pas pu être lancé. Réessayez.');setLoading(null)}
  }
