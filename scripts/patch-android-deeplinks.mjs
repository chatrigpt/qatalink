...
fs.copyFileSync('public/qatalink-logo.png',path.join(drawableDir,'qatalink_logo.png'));
xml=xml.replace(/android:icon="@[^"]+"/,'android:icon="@drawable/qatalink_logo"');
xml=xml.replace(/android:roundIcon="@[^"]+"/,'android:roundIcon="@drawable/qatalink_logo"');

if(!xml.includes('QatalinkBackgroundLocationService')){
  xml=xml.replace('</application>',`        <service android:name=".QatalinkBackgroundLocationService" android:foregroundServiceType="location" android:exported="false" />\n    </application>`);
}
...
public class QatalinkBackgroundLocationService extends Service implements LocationListener {
    ...
    private void startLocationUpdates() {
        if (!hasPermission() || locationManager == null) { stopTracking(); return; }
        try {
            locationManager.removeUpdates(this);
            if (locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER))
                locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 4000L, 2f, this, Looper.getMainLooper());
            if (locationManager.isProviderEnabled(LocationManager.NETWORK_PROVIDER))
                locationManager.requestLocationUpdates(LocationManager.NETWORK_PROVIDER, 6000L, 3f, this, Looper.getMainLooper());
    ...
    @Override public void onLocationChanged(@NonNull Location location) {
        ...
        long now = System.currentTimeMillis(); if (now - lastSentAt < MIN_SEND_MS) return; lastSentAt = now;
    ...
    private void postLocation(double lat, double lng, float accuracy, float bearing, float speed) {
        ...
        try {
            ...
            body.put("token", token); body.put("action", "location"); body.put("lat", lat); body.put("lng", lng);
            ...
            if (code == 400 || code == 404 || code == 410) stopSelf();
...
console.log('Qatalink Android icon, permissions, deep links and background delivery service registered');
