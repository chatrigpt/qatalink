import {cleanDeliveryLabel,orderMoney,receiptCatalogUrl,type PrintableOrder} from '@/lib/order-receipt';
...
async function shareReceiptImage(order:PrintableOrder,opts:PrinterOptions){
  ...
  const width=384,padding=14,work=document.createElement('canvas');work.width=width;work.height=3200;const ctx=work.getContext('2d');if(!ctx)throw new Error('CANVAS_UNAVAILABLE');ctx.fillStyle='#fff';ctx.fillRect(0,0,work.width,work.height);ctx.fillStyle='#000';ctx.textBaseline='top';let y=18;
  ...
  const catalogName=clean(opts.catalogTitle||opts.receiptTitle||opts.businessName||'Qatalink'),catalogUrl=receiptCatalogUrl(opts),delivery=cleanDeliveryLabel(order.delivery_address);
  ...
  if(catalogUrl){divider();const qrSize=150,qrX=(width-qrSize)/2;await drawQr(ctx,catalogUrl,qrX,y,qrSize);y+=qrSize+7;center('Scanner pour ouvrir le catalogue',14,true,1);center(catalogUrl,11,false,2)}divider();center(clean(opts.receiptFooter||'Commande enregistree avec Qatalink'),15,false,0);y+=14;
...
export async function printEscPosReceipt(order:PrintableOrder,opts:PrinterOptions){
  ...
  const port=await ensureEscPosPrinter(opts.baudRate||9600,true);if(!port?.writable)throw new Error('PRINTER_NOT_CONNECTED');const currency=order.currency_code||'XOF',chunks:Uint8Array[]=[],catalogName=clean(opts.catalogTitle||opts.receiptTitle||opts.businessName||'Qatalink'),catalogUrl=receiptCatalogUrl(opts),delivery=cleanDeliveryLabel(order.delivery_address);
  chunks.push(cmd(0x1b,0x40)); // initialise only: no forced code page
  ...
  if(catalogUrl){chunks.push(line('--------------------------------'),cmd(0x1b,0x61,0x01),...qrCommands(catalogUrl,5),line(),line('Scanner pour ouvrir le catalogue'));for(const l of wrap(catalogUrl))chunks.push(line(l))}
