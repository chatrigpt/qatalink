...
function navBluetooth(){return typeof navigator!=='undefined'?(navigator as any).bluetooth:null}
...
export function escPosDirectSupported(){return (!!navSerial()||!!navBluetooth())&&typeof window!=='undefined'&&window.isSecureContext}
...
function makeBlePort(device:any,characteristic:any){
  const writeChunk=async(data:Uint8Array)=>{
    ...
    for(let i=0;i<data.length;i+=size){
      ...
      else if(typeof characteristic.writeValue==='function')await characteristic.writeValue(chunk);
      else throw new Error('BLE_CHARACTERISTIC_NOT_WRITABLE');
      if(data.length>size)await new Promise(resolve=>setTimeout(resolve,12));
    }
  };
  return {writable:{getWriter(){return {write:writeChunk,async close(){},releaseLock(){}}}},async close(){try{device.gatt?.disconnect()}catch{}}};
}

async function findBleWritable(server:any){
  for(const serviceId of BLE_SERVICES){
    try{
      const service=await server.getPrimaryService(serviceId);
      const chars=await service.getCharacteristics();
      const writable=chars.find((c:any)=>c.properties?.writeWithoutResponse||c.properties?.write);
      if(writable)return writable;
    }catch{}
  }
  throw new Error('BLE_PRINTER_PROFILE_UNSUPPORTED');
}

async function connectBlePrinter(){
  const bluetooth=navBluetooth();if(!bluetooth)throw new Error('WEB_BLUETOOTH_UNSUPPORTED');
  let device:any=null;
  try{
    const granted=typeof bluetooth.getDevices==='function'?await bluetooth.getDevices().catch(()=>[]):[];
    device=granted.find((d:any)=>d?.gatt)||null;
    if(!device)device=await bluetooth.requestDevice({acceptAllDevices:true,optionalServices:BLE_SERVICES});
    const server=device.gatt?.connected?device.gatt:await device.gatt?.connect();
    if(!server)throw new Error('BLE_CONNECTION_FAILED');
    const characteristic=await findBleWritable(server);
    activeBleDevice=device;activePort=makeBlePort(device,characteristic);
    device.addEventListener?.('gattserverdisconnected',()=>{if(activeBleDevice===device){activeBleDevice=null;activePort=null}});
    return activePort;
  }catch(e){if(device?.gatt?.connected)try{device.gatt.disconnect()}catch{}throw e}
}
...
export async function connectEscPosPrinter(baudRate=9600){
  ...
  if(serial){let port:any=null;const granted=await serial.getPorts().catch(()=>[]);if(granted?.length===1)port=granted[0];if(!port)port=await serial.requestPort();if(!port.writable)await port.open({baudRate});activePort=port;return true}
  if(navBluetooth()){await connectBlePrinter();return true}
  throw new Error('DIRECT_PRINT_UNSUPPORTED');
}
export async function disconnectEscPosPrinter(){if(!activePort)return;try{if(activePort.writable){const writer=activePort.writable.getWriter();await writer.close().catch(()=>{});writer.releaseLock?.()}}catch{}try{await activePort.close()}catch{}activePort=null;activeBleDevice=null}
export async function ensureEscPosPrinter(baudRate=9600,requestIfMissing=false){
  if(activePort?.writable)return activePort;
  const serial=navSerial();
  if(serial){const ports=await serial.getPorts().catch(()=>[]);if(ports?.length){activePort=ports[0];if(!activePort.writable)await activePort.open({baudRate});return activePort}if(requestIfMissing){const port=await serial.requestPort();if(!port.writable)await port.open({baudRate});activePort=port;return activePort}}
  if(navBluetooth()&&requestIfMissing)return connectBlePrinter();
  throw new Error('PRINTER_NOT_CONNECTED');
}

export async function printEscPosReceipt(order:PrintableOrder,opts:PrinterOptions){
  let port:any=null;
  try{port=await ensureEscPosPrinter(opts.baudRate||9600,true)}catch(e){if(mobileShareAvailable()){await shareReceiptImage(order,opts);return}throw e}
  if(!port?.writable)throw new Error('PRINTER_NOT_CONNECTED');const currency=order.currency_code||'XOF',chunks:Uint8Array[]=[],catalogName=clean(opts.catalogTitle||opts.receiptTitle||opts.businessName||'Qatalink'),catalogUrl=receiptCatalogUrl(opts),delivery=orderDeliveryLabel(order);
