...
export function printOrderReceipt(order:PrintableOrder,opts:{businessName:string;catalogTitle?:string;receiptTitle?:string|null;receiptFooter?:string|null;width?:'58mm'|'80mm'}){
  ...
  if(!w)return;
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(order.order_number)}</title><style>@page{size:${width} auto;margin:3mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;width:${width};margin:0 auto;padding:3mm;color:#111;font-size:11px}.center{text-align:center}.title{font-size:16px;font-weight:800}.muted{color:#555}.rule{border-top:1px dashed #111;margin:8px 0}.line{display:flex;justify-content:space-between;gap:8px;margin:7px 0}.line>div{min-width:0}.line small{display:block;font-size:9px;margin-top:2px}.line strong{text-align:right;white-space:nowrap}.total{display:flex;justify-content:space-between;font-size:15px;font-weight:800;margin-top:8px}.note{white-space:pre-wrap}.footer{margin-top:12px;text-align:center;font-size:9px}.no-print{display:block;margin:12px auto;padding:8px 12px}@media print{.no-print{display:none}}</style></head><body><div class="center"><div class="title">${esc(opts.receiptTitle||opts.businessName)}</div>${opts.catalogTitle?`<div>${esc(opts.catalogTitle)}</div>`:''}<div class="muted">Commande ${esc(order.order_number)}</div><div class="muted">${esc(new Date(order.created_at).toLocaleString('fr-FR'))}</div></div>${meta?`<div class="rule"></div><div>${meta}</div>`:''}<div class="rule"></div>${lines}<div class="rule"></div>${order.total_minor!==null&&order.total_minor!==undefined?`<div class="total"><span>TOTAL</span><span>${esc(orderMoney(order.total_minor,currency))}</span></div>`:''}${order.customer_note?`<div class="rule"></div><div class="note"><b>Note :</b><br/>${esc(order.customer_note)}</div>`:''}<div class="footer">${esc(opts.receiptFooter||'Commande enregistrée avec Qatalink')}</div><button class="no-print" onclick="window.print()">Imprimer</button><script>setTimeout(()=>window.print(),250)</script></body></html>`);
  w.document.close();
}
