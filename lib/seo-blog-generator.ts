...
export async function generateAndPublishSeoArticle(opts:{bootstrap?:boolean}={}){
  ...
  const modelContext={requested_model:PRIMARY_MODEL,topic_key:topic.topic_key,city:city?.name||null,generated_at:new Date().toISOString(),publisher_mode:privileged?'service_role':'limited_rpc'};
  ...
  if(!privileged){
    ...
    return {skipped:false,post:{id:published?.id,slug:published?.slug||slug,title:published?.title||title,model:published?.model||actualModel,quality_score:Number(published?.quality_score??score),city:published?.city??city?.name??null}};
  }
